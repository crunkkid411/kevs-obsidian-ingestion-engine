...
| **Investigator GUI** | **native: `egui` + `libmpv`** (or Qt + libmpv) | Rust (or C++/Qt) | — | frame-accurate mpv video; talks to the DB + query agent in-process; clip/stitch via auto-editor. **No web** (browser video stacks are unreliable for this). See `docs/GUI.md`. |
| **UI self-verification** | **screenshot + Claude vision + Win32** | Windows-MCP / computer-use-mcp | ShowUI-2B (grounding, optional) | drive the real app, read the screen, click/type, verify, fix own failures (BUILD.md Phase 6). |
